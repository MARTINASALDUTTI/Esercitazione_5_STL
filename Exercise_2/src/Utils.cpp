...
bool ImportMesh(PolygonalMesh& mesh)
{
    ...
    else
    {
        ...
        //TEST: the edges have no-zero lenght
        for (const auto& edge : mesh.Cell1DVertices)
        {
            ...
            {
                std::cerr << "there is an edge with zero lenght"<< std::endl;
                return 1;
            }
        }
    }
    ...
    {
        //TEST: the cell2D has no-zero area
        for(unsigned int c = 0; c < mesh.NumberCell2D; c++)
        {
            ...
            for (unsigned int i = 0; i< n-1; i++)
                area += 0.5 * abs(((mesh.Cell0DCoordinates[vertices[i]][0]*mesh.Cell0DCoordinates[vertices[i+1]][1]) -
                                   (mesh.Cell0DCoordinates[vertices[i+1]][0]*mesh.Cell0DCoordinates[vertices[i]][1])));
            if (area < sqrt(3)/4*(1.0e-16*1.0e-16))
            {
                std::cerr << "there is a cell with zero area"<< std::endl;
                return 2;
            }
        }
    }
